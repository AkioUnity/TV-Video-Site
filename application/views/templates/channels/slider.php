        ...
        if ($sliders) {
            foreach ($sliders as $set_slider) {
                ...
                <div class="ms-slide">
                    ...
                    <img src="assets/frontends/vendors/masterslider/style/blank.gif"
                         data-src="<?= !empty($set_slider->image) ? 'assets/uploads/channels/full/' . $set_slider->image : 'assets/frontends/images/Hero2.jpg' ?>"/>
                    <?php
                    if (!empty($set_slider->video_link) && strpos($set_slider->video_link, 'amazonaws.com') == false) {
                        $video_class = 'video-text-bg';
                        ...
                        ?>
                        <iframe src="<?= $videoLink ?>" webkitallowfullscreen mozallowfullscreen frameborder="0"
                                allowfullscreen style="width:100%;height:100%" data-mute="true"
                                data-loop="true">
                        </iframe>
                        <?php
                    } elseif (!empty($set_slider->video_file)) {
                        ...
                        <video data-autopause="false" data-mute="true" data-loop="true" data-fill-mode="fill">
                            <source id="mp4-<?= $set_slider->id ?>"
                                    src="<?= $set_slider->video_file ?>" type="video/mp4"/>
                        </video>
                        <?php
                    }
                    ?>
                    <div class="ms-layer" data-effect="bottom(45)" data-duration="300" data-ease="easeInOut"
                         data-origin="bl">
                        <div class="meta animate-element" data-anim="fadeInUp">
                            <span class="author"><img
                                        src="<?= !empty($channel_data->logo) ? 'assets/uploads/channels/full/' . $channel_data->logo : 'assets/frontends/channels/images/coronis-C.png' ?>"
                                        width="100px"/></span>
                        </div>
                        <a href="javascript:;"><h2 class="animate-element"
                                                   data-anim="fadeInUp"><?= $set_slider->name ?></h2></a>
                        ...
                        <div class="animate-element" data-anim="fadeInUp">
                            ...
                            if (!empty($set_slider->video_link)) {
                                ?>
                                <a class="video-player video-player-center video-player-large button beauty-hover"
                                   href="<?= $set_slider->video_link ?>">Watch Now</a>
                                <?php
                            }
                            ?>
                            <!--                    <a href="javascript:;" class="button beauty-hover"><i class="fa fa-play"></i> Watch Now</a>-->
                            <a href="javascript:;" class="button beauty-hover s-btn-more"><i
                                        class="fa fa-ellipsis-h"></i> View More</a>
                            <?php
                            if ($subscribeBtn) {
                                ?>
                                <a href="javascript:;" class="button beauty-hover btn-subscribeBtn"
                                   onclick="set_subscribe()">Subscribe </a>
                                <?php
                            }
                            ?>
                        </div>
