<?php
class News_blazers extends Admin_Controller{
	public $_table_names = 'news';		//set table name
	public $_subView = 'admin/news/';	//set subview load 
	public $_redirect = '/news_blazers';			//set link with controller file name
    ...
    public function index(){
        ...
        $this->data['all_data'] = $this->comman_model->get_by($this->_table_names,array('section'=>'Blazers'),false);

		//set lead view		
        $this->data['subview'] = $this->_subView.'index_blazers';	
	...
	public function set_section(){
		...
		if($id){
			$account_data = $this->comman_model->get_by($this->_table_names,array('id'=>$id),false,true);
			if($account_data){
				$output['status']= 'ok';
				$output['msge']= '';
				$this->db->trans_start();
				$this->db->where('id',$account_data->id);
				$this->db->set('order',$value,true);
				$this->db->update($this->_table_names);
				$this->db->trans_complete();
			}
			else{
				$output['msge']= 'There is no news';
			}
		}
    ...
    public function edit($id = NULL){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $data =array();
			$postArr = array('name','video_file','author_id','description','code','label','external_url','link','order','tags');
...
/*	   	$this->db->order_by('name','asc');
		...
		$this->data['subview'] = $this->_subView.'edit_blazers';
        $this->load->view('admin/_layout_main', $this->data);
	}
