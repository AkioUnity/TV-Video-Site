...
class Channel_request extends Frontend_Controller{	
	public $_redirect = 'user/channel_request';

	public $_subView = 'user/channel_request/';
	...
	function index(){
		$this->data['form_data'] = $form_data = $this->comman_model->get_by('users_channels_request',array('user_id'=>$this->data['user_details']->id),false,true);
        ...
        if($this->form_validation->run() == TRUE){
			$post1 =array('first_name','last_name','website','company_name','friends','details');
        	$post_data = $this->comman_model->array_from_post($post1);

			$post_data['options'] = json_encode($this->input->post('options'));
        ...
        $this->data['name'] = 'Channel Request';
